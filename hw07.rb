...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    # when self is the intersection between an object and the segment as line
    # if the point is in the segment return the point, else no points
...
class Line < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    # when self is the intersection between an object and the segment as line
    # if the intersection is a line return the segment
    seg # buggy!
  end
...
class VerticalLine < GeometryValue
  ...
  def intersectWithSegmentAsLineResult seg
    # when self is the intersection between an object and the segment as line
    # if the intersection is a vertical line return the segment
    seg
  end
end
...
class LineSegment < GeometryValue
  ...
  def intersectLine l1
    l1.intersectLineSegment self
  end
  ...
  def intersectWithSegmentAsLineResult seg
    # when self is the intersection between an object and the segment as line
    # the hard case in the hard case
    ...
    segs = [[self.x1, self.y1, self.x2, self.y2],
            [seg.x1, seg.y1, seg.x2, seg.y2]]
    
    if real_close(self.x1, self.x2)
      # they are on a vertical line
      # segment a is the lower one
      segs.reverse! if self.y1 > seg.y1  # reverse in place

      (aXstart, aYstart, aXend, aYend),
      (bXstart, bYstart, bXend, bYend) = segs
      
      # a is the lower segment
      ...
      # the segments are alligned
      segs.reverse! if self.x1 > seg.x1  # reverse in place
      ...
      if real_close(aXend, bXstart)
        ...
        LineSegment.new(bXstart, bYstart, aXend, aYend) # b inside a
      end
    end
  end
  ...
  private
  def inverted?
  # is x1, y1 to the right of x2, y2?
    (x1-x2 > GeometryExpression::Epsilon) ||
       (real_close(x1,x2) && y1-y2 > GeometryExpression::Epsilon)
  end
...
class Intersect < GeometryExpression
  ...
  def preprocess_prog
    Intersect.new(@e1.preprocess_prog, @e2.preprocess_prog) # untested
  end
...
class Let < GeometryExpression
  ...
  def eval_prog env
    new_binding = [@s, @e1.eval_prog(env.dup)]
    @e2.eval_prog([new_binding] + env)
  end
